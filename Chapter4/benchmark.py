...
if __name__ == "__main__":
    ...
    assert config["openai_api_key"] is not None, "OpenAI API Key is required, set the OPENAI_API_KEY environment variable."
    assert config["huggingface_token"] is not None, "HuggingFace API Token is required, set the HUGGINGFACEHUB_API_TOKEN environment variable."
    ...
    # Initialize models
    llm_gpt3 = OpenAI(model_name=config["openai_model_name"], temperature=0.9, max_tokens=256)
    llm_neo = HuggingFaceHub(repo_id=config["huggingface_repo_id"], model_kwargs={"temperature": 0.9})

    # Generate descriptions
    gpt3_descriptions = generate_descriptions(llm_gpt3, reference_descriptions, config["prompt_template"])
    gptneo_descriptions = generate_descriptions(llm_neo, reference_descriptions, config["prompt_template"])

    # Save generated descriptions
    gpt3_descriptions_df = pd.DataFrame({"product_description": gpt3_descriptions, "product_image": product_images})
    gpt3_descriptions_df.to_csv("data/gpt3_descriptions.csv", index=False)

    gptneo_descriptions_df = pd.DataFrame({"product_description": gptneo_descriptions, "product_image": product_images}) 
