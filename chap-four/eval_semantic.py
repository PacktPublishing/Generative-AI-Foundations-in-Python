...
def cosine_similarity(model, reference_descriptions, generated_descriptions):
    ...
    cosine_scores = [util.pytorch_cos_sim(model.encode(ref), model.encode(gen))[0][0] for ref, gen in zip(reference_descriptions, generated_descriptions)]
...
if __name__ == "__main__":
    ...
    average_cosine_gpt3 = cosine_similarity(model, reference_descriptions, generated_descriptions_gpt3)
    print(f"Average Cosine Similarity GPT-3: {average_cosine_gpt3}")

    average_cosine_neo = cosine_similarity(model, reference_descriptions, generated_descriptions_neo)
