...
def process_inputs(processor: CLIPProcessor, prompts: List[str], images: List[Image.Image]) -> dict:
    processed_texts = processor(text=prompts, padding=True, truncation=True, max_length=77, return_tensors="pt")
    processed_images = processor(images=images, return_tensors="pt")
    return {"input_ids": processed_texts["input_ids"], "attention_mask": processed_texts["attention_mask"], "pixel_values": processed_images["pixel_values"]}
...
if __name__ == "__main__":
    ...
    reference_images = [load_image_from_path(row[config["image_column_name"]]) for _, row in reference_data.iterrows() if row[config["image_column_name"]] is not None]
    
    gpt3_descriptions = pd.read_csv(config["generated_descriptions_gpt3_path"])[config["description_column_name"]].tolist()
    gptneo_descriptions = pd.read_csv(config["generated_descriptions_neo_path"])[config["description_column_name"]].tolist()
    reference_descriptions = reference_data[config["description_column_name"]].tolist()

    gpt3_generated_scores = clip_scores(reference_images, gpt3_descriptions, clip_model, clip_processor)
    gptneo_generated_scores = clip_scores(reference_images, gptneo_descriptions, clip_model, clip_processor)
    reference_scores = clip_scores(reference_images, reference_descriptions, clip_model, clip_processor)
